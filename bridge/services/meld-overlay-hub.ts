...
import type { MeldOverlayConfig } from '../../schemas/config.js';
import { MELD_OVERLAY_CONTRACT_VERSION, projectMeldOverlayEvent } from '../core/meld-overlay.js';
import type { Logger } from './logger.js';

export class MeldOverlayHub {
  ...
  public constructor(private readonly logger: Logger, private readonly config: MeldOverlayConfig) {
    this.sockets.on('connection', (socket) => {
      socket.send(JSON.stringify({ contractVersion: MELD_OVERLAY_CONTRACT_VERSION, kind: 'hub.ready', emittedAt: new Date().toISOString() }));
      this.logger.info('Meld overlay client connected', { clients: this.sockets.clients.size });
      socket.on('close', () => this.logger.info('Meld overlay client disconnected', { clients: this.sockets.clients.size }));
    });
  }

  public attach(server: Server): void {
    if (this.attachedServer === server) return;
    if (this.attachedServer !== undefined) throw new Error('Meld overlay hub is already attached');
  ...
  public publish(event: NormalizedEvent): void {
    ...
    const overlayEvent = projectMeldOverlayEvent(event);
  ...
  public clientConfig(): MeldOverlayConfig { return { ...this.config }; }
