...
export const moduleManifestV2Schema = z.object({
  contractVersion: z.literal(CORE_CONTRACT_VERSION), moduleId: identifierSchema, name: z.string().min(1).max(200), version: semverSchema, minimumCoreVersion: z.string().min(1).max(50), maximumTestedCoreVersion: z.string().min(1).max(50), minimumBridgeVersion: semverSchema.optional(), maximumTestedBridgeVersion: semverSchema.optional(), dependencies: z.array(identifierSchema).max(50).default([]), requiredCapabilities: z.array(platformCapabilityIdSchema).max(50).default([]), configurationSchema: z.string().min(1).max(500), eventSubscriptions: z.array(identifierSchema).max(100).default([]), commandsProvided: z.array(providedObjectSchema).max(100).default([]), actionsProvided: z.array(providedObjectSchema).max(100).default([]), browserSourcesProvided: z.array(providedObjectSchema.extend({ path: z.string().startsWith('/').max(500) }).strict()).max(50).default([]), coordination: z.array(coordinationSchema).max(20).optional(), dataStorageOwned: z.array(z.string().min(1).max(500)).max(100).default([]), installationSteps: z.array(z.string().min(1).max(1_000)).max(100), uninstallationSteps: z.array(z.string().min(1).max(1_000)).max(100), migrations: z.array(migrationSchema).max(100).default([]), healthChecks: z.array(z.object({ id: identifierSchema, description: z.string().min(1).max(500) }).strict()).max(50).default([]),
  ...
  if ((manifest.minimumBridgeVersion === undefined) !== (manifest.maximumTestedBridgeVersion === undefined)) context.addIssue({ code: 'custom', path: ['minimumBridgeVersion'], message: 'Bridge release compatibility must declare both its minimum and maximum tested versions.' });
