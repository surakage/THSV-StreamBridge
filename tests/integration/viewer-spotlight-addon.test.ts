...
describe('Viewer Spotlight integration', () => {
  it('publishes a bounded card through both privacy-preserving providers without persisting presentation identity', async () => {
    ...
    const spotlight = { ...viewerSpotlight, settings: { enabled: true, disclosureAccepted: true, rewardRequestsEnabled: true, rewardId: 'spotlight-reward', showObservedCommands: true }, capabilityGrant: { moduleId: 'thsv.viewer-spotlight', permissions: ['events.subscribe', 'state.private', 'overlay.publish', 'schedule.bounded', 'viewer.foundation.read', 'viewer.foundation.mutate', 'community.analytics.read'] as const, approvedActionIds: [] } };
    const registry = new ModuleRegistry([spotlight, analytics, foundation], silentLogger, 5_000, broker); await registry.start();
    const reward: NormalizedEvent = { schemaVersion: '1.0.0', eventId: 'card-one', eventType: 'reward.redemption', platform: 'twitch', source: { adapter: 'fixture', eventId: 'card-one', eventName: 'Reward Redemption' }, receivedAt: '2026-07-26T12:00:00.000Z', channel: { name: 'channel' }, user: { id: '123456', name: 'alex_login', displayName: 'Alex Display', avatarUrl: 'https://example.com/alex.png', actorType: 'human', roles: [] }, payload: { rewardId: 'spotlight-reward', redemptionId: 'spotlight-redemption', verifiedTransport: true, supportedOperations: [] }, metadata: { simulated: false } };
    await registry.publish(reward);
    expect(published).toEqual([{ moduleId: 'thsv.viewer-spotlight', topic: 'thsv.viewer-spotlight.card.show', payload: expect.objectContaining({ title: 'Alex Display • Twitch', text: '25 points • Level 1 • 0 observed commands', imageUrl: 'https://example.com/alex.png' }) }]);
  ...
  it('isolates an overlay publication failure and continues delivering the event to unrelated modules', async () => {
    ...
    const foundation = { ...viewerFoundation, settings: { enabled: true, accountLinks: ['alex|twitch|123456'], chatCooldownSeconds: 0 }, capabilityGrant: { moduleId: 'thsv.viewer-foundation', permissions: ['events.subscribe', 'state.private', 'viewer.foundation.provide'] as const, approvedActionIds: [] } };
    const analytics = { ...communityAnalytics, settings: { enabled: true }, capabilityGrant: { moduleId: 'thsv.community-analytics', permissions: ['events.subscribe', 'state.private', 'viewer.foundation.read', 'community.analytics.provide'] as const, approvedActionIds: [] } };
    const spotlight = { ...viewerSpotlight, settings: { enabled: true, disclosureAccepted: true, rewardRequestsEnabled: true, rewardId: 'spotlight-reward' }, capabilityGrant: { moduleId: 'thsv.viewer-spotlight', permissions: ['events.subscribe', 'state.private', 'overlay.publish', 'schedule.bounded', 'viewer.foundation.read', 'viewer.foundation.mutate', 'community.analytics.read'] as const, approvedActionIds: [] } };
    ...
    const reward: NormalizedEvent = { schemaVersion: '1.0.0', eventId: 'card-overlay-failure', eventType: 'reward.redemption', platform: 'twitch', source: { adapter: 'fixture', eventId: 'card-overlay-failure', eventName: 'Reward Redemption' }, receivedAt: '2026-07-26T12:00:00.000Z', channel: { name: 'channel' }, user: { id: '123456', name: 'alex_login', displayName: 'Alex Display', actorType: 'human', roles: [] }, payload: { rewardId: 'spotlight-reward', redemptionId: 'spotlight-redemption-failure', verifiedTransport: true, supportedOperations: [] }, metadata: { simulated: false } };
    await expect(registry.publish(reward)).resolves.toBeUndefined();
