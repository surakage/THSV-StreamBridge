...
test('wizard installs and configures add-ons without injecting package code', async ({ page, context }) => {
  ...
  await expect(page.getByRole('article').getByText('User Translate 2.5.1', { exact: true })).toBeVisible();
  ...
  const quickStartSection = userTranslateSettings.locator('.addon-settings-section').filter({ hasText: 'Quick start' });
  ...
  await expect(page.locator('[data-addon-settings="thsv.user-translate"] .addon-settings-section').filter({ hasText: 'Quick start' })).not.toHaveAttribute('open', '');
  await userTranslateSettings.locator('summary').filter({ hasText: 'Privacy and provider' }).click();
  ...
  await expect(page.getByRole('article').getByText('Auto Translate 2.5.1', { exact: true })).toBeVisible();
  ...
  await autoTranslateSettings.locator('summary').filter({ hasText: 'Audience' }).click();
  await expect(autoTranslateSettings.getByLabel('Who may be translated')).toHaveValue('allowlist-only');
  await autoTranslateSettings.locator('summary').filter({ hasText: 'Safety limits' }).click();
  await expect(autoTranslateSettings.getByLabel('Maximum requests waiting')).toHaveValue('5');
  await autoTranslateSettings.locator('summary').filter({ hasText: 'Chat rate limits' }).click();
  await expect(autoTranslateSettings.getByLabel('Maximum translated chat percentage')).toHaveValue('25');
  await autoTranslateSettings.locator('summary').filter({ hasText: 'Review before enabling' }).click();
