...
test('release readiness combines lifecycle, PR checks, post-release evidence, and protected approvals without publishing', async ({ page }) => {
  await page.route('**/wizard/api/release-readiness', async (route) => await route.fulfill({ contentType: 'application/json', body: JSON.stringify({ version: '4.0.3', checkedAt: '2026-08-20T12:00:00.000Z', githubStatusSource: 'cache', usingCachedGitHubStatus: true, localLifecycle: { currentTag: 'v4.0.3', previousTag: 'v4.0.2', previousChecksumVerified: true, previousProvenanceVerified: true, creatorDataPreserved: true }, pullRequest: { available: false, message: 'Refresh GitHub status to inspect the open release-candidate PR.' }, checks: [], postReleaseSmoke: { available: false, message: 'Post-release verification begins automatically after publication.' }, summary: { lifecycleReady: true, checksGreen: false, postReleaseVerified: false, readyForCreatorReview: false }, remainingCreatorApprovals: ['Review and merge the pull request.', 'Approve the protected streambridge-tag environment deployment.', 'Approve the protected streambridge-release environment deployment.'] }) }));
  await page.route('**/wizard/api/release-readiness/refresh', async (route) => await route.fulfill({ contentType: 'application/json', body: JSON.stringify({ version: '4.0.3', checkedAt: '2026-08-21T12:00:00.000Z', githubStatusSource: 'live', usingCachedGitHubStatus: false, localLifecycle: { currentTag: 'v4.0.3', previousTag: 'v4.0.2', previousChecksumVerified: true, previousProvenanceVerified: true, creatorDataPreserved: true }, pullRequest: { available: true, number: 9, title: 'Prepare StreamBridge 4.0.3', url: 'https://github.com/surakage/THSV-StreamBridge/pull/9', branch: 'codex/release-4.0.3-seamless' }, checks: [{ name: 'windows', status: 'completed', conclusion: 'success' }], postReleaseSmoke: { available: true, tag: 'v4.0.3', previousTag: 'v4.0.2', conclusion: 'success', reinstall: '4.0.3', rollbackProtectionVerified: true, creatorDataPreserved: true, url: 'https://github.com/surakage/THSV-StreamBridge/actions/runs/91', evidenceUrl: 'https://github.com/surakage/THSV-StreamBridge/actions/runs/91#artifacts' }, summary: { lifecycleReady: true, checksGreen: true, postReleaseVerified: true, readyForCreatorReview: true }, remainingCreatorApprovals: ['Review and merge the pull request.', 'Approve the protected streambridge-tag environment deployment.', 'Approve the protected streambridge-release environment deployment.'] }) }));
  await unlock(page);
