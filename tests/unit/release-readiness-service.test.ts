...
describe('ReleaseReadinessService', () => {
  it('combines lifecycle evidence with public PR checks without repository mutation', async () => {
    const root = await mkdtemp(join(tmpdir(), 'thsv-release-readiness-')); roots.push(root); const evidence = join(root, 'latest.json'); const published = join(root, 'published.json');
    ...
    const service = new ReleaseReadinessService('4.0.3', evidence, published, 'surakage/THSV-StreamBridge', fetcher);
    ...
    expect(refreshed).toMatchObject({ pullRequest: { available: true, number: 9 }, postReleaseSmoke: { available: true, conclusion: 'success', evidenceUrl: 'https://github.test/actions/91#artifacts' }, summary: { lifecycleReady: true, checksGreen: true, postReleaseVerified: true, readyForCreatorReview: true } });
  ...
  it('prefers current local post-release evidence with rollback protection', async () => {
    const root = await mkdtemp(join(tmpdir(), 'thsv-release-published-')); roots.push(root); const lifecycle = join(root, 'lifecycle.json'); const published = join(root, 'published.json');
    ...
    const service = new ReleaseReadinessService('4.0.3', lifecycle, published, 'surakage/THSV-StreamBridge', vi.fn<typeof fetch>());
