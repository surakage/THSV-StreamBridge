...
function runtime(settings: Record<string, unknown> = {}) {
  ...
  const context = {
    ...
    viewerFoundation: { getProjection: vi.fn(async (query: Record<string, unknown>) => query.viewerId === undefined
      ? { contractVersion: '1.0.0', viewerId: 'viewer-one', linked: false, points: 245, level: 3, nextLevelAt: 300 }
      : { contractVersion: '1.0.0', viewerId: query.viewerId, linked: false, points: 245, level: 3, nextLevelAt: 300 }) },
    communityAnalytics: { getViewerProjection: vi.fn(async (viewerId: string): Promise<Record<string, unknown>> => ({ contractVersion: '1.0.0', viewerId, observed: true, firstSeenAt: 1, lastSeenAt: 2, sessions: 4, counters: { messages: 12, commands: 3, follows: 0, subscriptions: 0, memberships: 0, giftSubscriptions: 0, gifts: 0, cheers: 0, superChats: 0, raids: 0, rewardRedemptions: 0 }, activeSession: true, activeLastSeenAt: 2 })) },
  };
...
describe('Viewer Spotlight add-on', () => {
  it('shows one self-requested card from re-read provider projections', async () => {
    ...
    expect(testRuntime.published).toEqual([{ topic: 'thsv.viewer-spotlight.card.show', payload: expect.objectContaining({ title: 'Viewer Name • Twitch', text: '245 points • Level 3 • 4 observed sessions • 12 observed messages', imageUrl: 'https://example.com/avatar.png' }) }]);
  });
