...
async function writePortableRelease(root: string, version: string, marker: string): Promise<void> {
  ...
  for (const name of ['start.mjs', 'start-streaming-tools.mjs', 'stop.mjs', 'open-wizard.mjs', 'uninstall.mjs', 'tray.ps1', 'tray-status.ps1', 'Start THSV StreamBridge.cmd', 'Start THSV Streaming Tools.cmd', 'Stop THSV StreamBridge.cmd', 'Open THSV Setup Wizard.cmd', 'Open THSV StreamBridge Tray.cmd', 'Uninstall THSV StreamBridge.cmd']) await copyFile(join('launcher', name), join(root, 'launcher', name));
  ...
  const paths = [
    ...
    'launcher/start.mjs', 'launcher/start-streaming-tools.mjs', 'launcher/stop.mjs', 'launcher/open-wizard.mjs', 'launcher/uninstall.mjs', 'launcher/tray.ps1', 'launcher/tray-status.ps1', 'launcher/start-streamerbot.mjs',
