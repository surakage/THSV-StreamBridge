...
describe('Raid Scout package files', () => {
  it('ships one stable triggerless controller, exact creator controls, and a provider stop confirmation', async () => {
    ...
    expect(manifest.actions.slice(1, 5).map((action) => action.arguments?.[0]?.value)).toEqual(['suggest', 'confirm', 'cancel', 'broadcast-stopped']);
    ...
    expect(manifest.actions[5]?.arguments).toBeUndefined();
