...
describe('Timed Message Output Streamer.bot package', () => {
  it('packages a triggerless concurrent multi-platform sender with simulation suppression', async () => {
    ...
    const manifest = JSON.parse(await readFile(`${root}/manifest.json`, 'utf8')) as {
      ...
      contract: { requiredInputArguments: string[]; optionalInputArguments: string[]; outputArguments: string[]; executionSafety: Record<string, unknown> };
    };
    ...
    for (const argument of [...manifest.contract.requiredInputArguments, ...manifest.contract.optionalInputArguments, ...manifest.contract.outputArguments]) expect(source).toContain(`"${argument}"`);
    expect(source).toContain('CPH.SendMessage(message, useBotAccount, allowAccountFallback)');
    expect(source).toContain('CPH.SendYouTubeMessageToLatestMonitored(message, useBotAccount, allowAccountFallback)');
    expect(source).toContain('CPH.SendKickMessage(message, useBotAccount, allowAccountFallback)');
