...
describe('wizard launcher package', () => {
  ...
  it('announces asynchronous results and supports a persistent system-aware color theme', async () => {
    ...
    expect(shell.match(/aria-live="polite"/g)).toHaveLength(39);
    expect(shell.match(/role="status"/g)).toHaveLength(36);
    ...
    expect(shell).toContain('id="website-companion-card"');
    expect(styles).toContain('#website-companion-card{display:none}');
    expect(script).not.toContain('websiteCompanion');
    ...
    expect(script).toContain('startupReportText(status.lastStartupReport)');
    expect(script).toContain('Last startup (${when})');
    expect(script).toContain('runWithStartupProgress');
    expect(script).toContain("status.lastStartupReport?.outcome==='in-progress'");
    expect(script).toContain('const exactBlockers=Array.isArray(readiness.blockers)');
    expect(script).toContain('renderReadinessActions');
    expect(script).toContain("api('/wizard/api/live-acceptance')");
    expect(script).toContain("fetch('/wizard/api/support-bundle'");
    expect(script).toContain("api('/wizard/api/support-bundle/preview')");
    expect(script).toContain("api('/wizard/api/obs-source-inventory'");
    expect(script).toContain("fetch('/wizard/api/pre-stream-report'");
    expect(script).toContain("api('/wizard/api/pre-stream-report/compare'");
    expect(script).toContain("'x-thsv-support-preview':state.supportBundlePreview.previewId");
    expect(script).toContain('Use detected source');
    expect(script).toContain('Use detected replacement');
    expect(script).toContain('Periodic recheck due');
    expect(script).toContain('Recheck due soon');
    ...
    expect(shell).toContain('Last-known-good local backups');
    expect(script).toContain("api('/wizard/api/configuration/backups')");
    expect(script).toContain("api('/wizard/api/configuration/backups/restore'");
    expect(shell).toContain('Clear crash pause');
    expect(script).toContain("api('/wizard/api/streamerbot-launcher/optional/circuit/reset'");
    expect(script).toContain("api('/wizard/api/preflight')");
    expect(script).toContain('Nothing will be saved until you confirm a candidate.');
    expect(script).toContain('broadcastAutomation');
    expect(script).toContain('Choose exact program files');
    expect(script).toContain('Create the desktop shortcut with these exact programs?');
    expect(shell).toContain('Export sanitized pre-stream report');
    expect(shell).toContain('Compare with earlier report');
    expect(shell).toContain('Keep up to five sanitized reports in this browser');
    expect(shell).toContain('Download reviewed bundle');
