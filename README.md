...
> **Release status:** `2.0.0-rc.2` is the current v2 public release candidate. Its automated suites and packaged Windows installer acceptance pass, while the final live high-impact provider-event matrix remains required before a stable v2 production release. High-impact operations without verified provider-stable IDs remain blocked. See the [production-readiness gate](docs/production-readiness.md). Stable `1.x` remains available from the official releases page.
