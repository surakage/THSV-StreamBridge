...
The wizard can check the official GitHub repository for a newer stable release. **Download & verify** checks the exact Windows archive's adjacent SHA-256 and tagged GitHub Actions provenance before writing it to private staging. It never silently replaces a running bridge.
...
Optional add-ons are separate downloads on the same GitHub Release as core. Each has its own versioned `.thsv-addon`, SHA-256 checksum, and build-provenance attestation, so creators can update core without silently installing features they did not choose. The Add-ons page checks the official release index on demand and reports updates, compatibility problems, publisher mismatches, unlisted packages, or revocations. **Download & verify update** authenticates the official outer release bundle, verifies both checksum layers and the inner package identity, then places the package in the inbox. The creator still reviews it and chooses **Verify and install** separately; code is never silently installed, enabled, or restarted. Future valid packages placed under the repository's `addons/` directory are discovered by release automation and published the same way.
