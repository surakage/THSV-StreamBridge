...
> **Release status:** `2.0.0-preview.1` is a public-preview candidate. Its automated suites pass, but the final clean-machine installer test and live high-impact event acceptance matrix are still required before a stable v2 production release. See the [production-readiness gate](docs/production-readiness.md). Stable `1.x` remains available from the official releases page.
