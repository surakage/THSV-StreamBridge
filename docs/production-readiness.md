...
- All `41` Streamer.bot packages were regenerated from reviewed source and indexed at `4.0.1` with size and SHA-256 metadata.
...
- The public installer upgraded the local managed installation to Version 4, preserved creator data, removed inactive application versions, and passed its startup health check.
...
Most creators install only `THSV-StreamBridge-4.0.1.zip`. Built-in extensions and integrations come with that archive. Only packages listed in the current `THSV-StreamBridge-AddOns-index.json` are optional downloads. Historical release assets are not installation guidance for Version 4.
