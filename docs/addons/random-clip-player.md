...
1. Download and extract `THSV-StreamBridge-AddOn-Random-Clip-Player-4.0.0.zip` from the same GitHub release as StreamBridge.
2. In **Setup Wizard > Add-ons**, install `THSV-Random-Clip-Player-4.0.0.thsv-addon` and review its permissions.
3. Import `Streamer.bot/THSV-StreamBridge-Random-Clip-Player-4.0.0.sb` in Streamer.bot.
4. Return to the wizard, configure the add-on, approve only the actions it needs, enable it, and restart StreamBridge when prompted.

### Add-on-specific steps

1. Import the bundled Streamer.bot/THSV-StreamBridge-Random-Clip-Player-4.0.0.sb into Streamer.bot.
