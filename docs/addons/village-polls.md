...
1. Download and extract `THSV-StreamBridge-AddOn-Village-Polls-4.0.11.zip` from the same GitHub release as StreamBridge.
2. In **Setup Wizard > Add-ons**, install `THSV-Village-Polls-4.0.11.thsv-addon` and review its permissions.
...
Package kind: **executable**. Requested permissions: `events.subscribe`, `state.private`, `chat.send`, `overlay.publish`, `schedule.bounded`.
