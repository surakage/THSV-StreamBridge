...
1. Download and extract `THSV-StreamBridge-AddOn-Viewer-Spotlight-2.5.1.zip` from the same GitHub release as StreamBridge.
2. In **Setup Wizard > Add-ons**, install `THSV-Viewer-Spotlight-2.5.1.thsv-addon` and review its permissions.
3. Import `Streamer.bot/THSV-StreamBridge-Viewer-Spotlight-2.5.1.sb` in Streamer.bot.
...
3. Create the card command through Command Sync with no generated response.
...
Package kind: **executable**. Requested permissions: `events.subscribe`, `state.private`, `overlay.publish`, `schedule.bounded`, `viewer.foundation.read`, `community.analytics.read`, `streamerbot.run-approved-action`.
