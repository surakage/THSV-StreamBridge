...
When this extension publishes visual output, use `http://127.0.0.1:8787/overlay/addons/thsv.clip-library-cache` in OBS, Meld, or Streamlabs. The wizard shows and copies the active URL with the configured bridge port. If the extension has no visual output, the hosted page remains idle.
