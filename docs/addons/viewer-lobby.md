...
1. Download and extract `THSV-StreamBridge-AddOn-Viewer-Lobby-4.0.11.zip` from the same GitHub release as StreamBridge.
2. In **Setup Wizard > Add-ons**, install `THSV-Viewer-Lobby-4.0.12.thsv-addon` and review its permissions.
3. Import `Streamer.bot/THSV-StreamBridge-Viewer-Lobby-4.0.12.sb` in Streamer.bot.
