...
The installer starts **THSV StreamBridge Tray** after the Bridge passes its health check. Double-click its Village icon to open the Setup Wizard, or right-click it for status, safe streaming-tool startup, Bridge start/stop, and the installed folder. The shell checks the local readiness endpoint every 5 seconds and shows a notification when readiness changes or a new startup failure is recorded. Closing the shell does not stop StreamBridge; reopen it with `Open THSV StreamBridge Tray.cmd` from the installed folder.
