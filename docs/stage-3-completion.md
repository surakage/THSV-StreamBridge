...
- Overview, Streamer.bot inventory, ownership, diagnostics, lock, and cancellation surfaces use ordinary accessible HTML controls. Inspection, diagnostics, and transaction updates use polite atomic live regions, and active inspections expose `aria-busy` until the result is rendered.
- The management wizard deliberately follows the operating-system light/dark preference. Dark remains the default when no preference is exposed; a contrast-reviewed light palette is selected through `prefers-color-scheme: light`. This decision applies to the management UI only and does not alter transparent browser overlays used on stream.
...
## Stage 4 mutation gates

- Before any mutation route exists, all writes must pass through one auditable mutation gateway whose real request counter replaces the Stage 3 zero invariant.
- A single active mutation lease must prevent simultaneous wizard tabs from committing overlapping changes. Read-only inspection may remain available in additional tabs.
- Every commit must re-read ownership immediately before writing, reject unrelated or changed objects, and provide backup plus rollback evidence for partial failure.
