...
After upgrading StreamBridge, manually refresh or reload each Browser Source once so the host picks up the new versioned overlay assets. In OBS/Streamlabs, keep both **Shutdown source when not visible** and **Refresh browser when scene becomes active** disabled for the Alerts source. Alerts use a persistent event connection; recreating the page on scene activation can discard an alert that arrives during the reload. Keep **Shutdown source when not visible** disabled for Chat as well when you want recent chat to remain visible across scene changes; destroying the source intentionally clears its privacy-bounded in-memory feed.
...
The wizard exposes a separate `/overlay/chat/dock` URL for an operator-facing multichat panel. Add it to OBS through **View > Docks > Custom Browser Docks**, or to any host that accepts a local browser/web panel. The dock displays the normalized shared feed and adds a composer with **All live chats**, Twitch, YouTube, Kick, and TikTok destinations based on the platforms enabled in StreamBridge. Twitch, YouTube, and Kick replies deliberately use the connected creator/broadcaster account with account fallback disabled. TikTok replies use the sender configured in TikFinity. A successful dock result means Streamer.bot accepted the action; platform display is not end-to-end acknowledged.

The composer is available only on the loopback-only dock page. StreamBridge gives that page a short-lived, HttpOnly session cookie on load, rejects cross-origin sends, limits message frequency, enforces each selected platform's character limit, and never exposes the wizard bearer token in the dock URL. It reuses StreamBridge's existing Streamer.bot output and overlay event connection rather than opening a platform connection of its own. The ordinary `/overlay/chat` Browser Source remains display-only.
...
- Twitch and YouTube native emotes use the structured emote ranges and HTTPS image URLs supplied by Streamer.bot. BTTV, FrankerFaceZ, and 7TV channel/global catalogs are fetched in the background, cached for 30 minutes, and matched locally; viewer names and message text are never sent to those services. Twitch supports BTTV, FFZ, and 7TV catalogs, YouTube supports BTTV and 7TV, and Kick supports 7TV plus any native metadata its Streamer.bot trigger supplies. Catalogs warm on `stream.online`; if the bridge starts after that event, the first supported chat message waits at most 1.25 seconds for a cold catalog while the fetch continues in the background. Alerts and add-on overlays never wait on emote providers. TikTok retains Unicode emoji and readable emote codes because TikFinity does not currently provide a stable documented image-range contract. Failed or unavailable images fall back to their original text code rather than leaving a blank message.
...
- The wizard can save four deliberately different designs: **Regular** uses a full conversation card and identity band, **Compact** uses a dense single-row conversation, **Minimal** uses a transcript-style line without avatar or role clutter, and **Classic Chat** places identity and text on one familiar native-chat line with no card background. It also controls a vertical stack or horizontal row; the edge new messages enter from; slide, fade, pop, or instant movement; left, centered, or right text; font family and size; text/card/canvas colors and opacity; transparent canvas mode; platform-label/profile-picture/badge visibility; and a case-insensitive ignored-name list. Horizontal mode fills the available browser-source width with fixed-proportion cards instead of stretching them. The interactive dock always uses a readable vertical feed while respecting the selected message style. Ignored names are filtered before browser publication but do not block commands or alter Streamer.bot chat.
...
- Transient card presentations use one bridge-wide bounded foreground queue. Core alerts, reward cards such as Viewer Spotlight, shoutouts, game results, giveaway cards, prize wheels, and temporary hydration reminder/redemption cards wait for the active presentation to finish and then observe `overlayGapMs` before the next begins. Queue acceptance is immediate; waiting for an earlier card never holds an add-on event handler open. Clip/media playback remains on its independent media lane and begins immediately after its own media-slot workflow allows it. Persistent chat, counters, labels, polls, timers, ad status, viewer queues, and real-time accessibility captions remain independent so they do not become stale or disappear while waiting. Exact wizard template previews use the preview lane and bypass the production queue.
- Alerts retain their own bounded priority and aggregation rules before entering the shared presentation queue. The default holds at most 20 waiting alerts; when full, it discards the oldest alert from the lowest available priority so a gift storm cannot grow browser memory indefinitely. A malformed card is skipped without freezing later alerts. Donations, cheers, Super Chats, and raids are high priority; subscriptions, memberships, gifts, and milestones are normal; follows are low. Creator profiles may override priority and duration, disable a type, restrict it to selected Twitch, YouTube, Kick, or TikTok sources, render bounded plain-text templates, play a local chime, choose slide, fade, pop, or instant entry/exit transitions, or combine queued gift quantities inside a short window. The default is a horizontal rectangle that slides down and later slides up. An empty platform selection means all supported platforms. Priority never infers or converts money.
...
The character-limit values are local overlay display caps, not claims about each platform's outbound chat API. The defaults are conservative and can be adjusted from 40 to 500 characters in the wizard. Disabling a platform event in chat does not disable its alert card or stop Streamer.bot from receiving the normalized event. Ko-fi uses its provider add-on; Streamlabs donations reuse the existing authenticated Streamer.bot WebSocket subscription. Both require stable provider source IDs before a live financial event can enter the alert/chat pipeline.
...
Enabled installed add-ons that declare `overlay.publish` receive a core-hosted source. Official visual add-ons use short creator-facing routes: `/overlay/shoutouts`, `/overlay/clips`, `/overlay/subathon`, and `/overlay/countdown`. The generic `/overlay/addons/<module-id>` form remains supported for third-party add-ons and backward compatibility. Stream Labels uses the generic host with a bounded `?label=` selector for individual persistent text labels or its combined panel. The wizard shows exact copyable URLs and can send a safe preview. The page shares the existing overlay `SharedWorker` and WebSocket when supported, filters every envelope to its canonical module ID, and renders only core-defined card, media, timer, and label commands. Add-on packages cannot supply HTML, CSS, or JavaScript to this route.

The Add-ons page separates saved package state from the currently running module. A visible **Restart required** state persists through wizard navigation and clears only after the wizard observes a new StreamBridge process start. Every add-on host registers its module and per-page renderer identity over the shared socket, so runtime health distinguishes “the overlay transport is connected” from “this exact add-on has a connected browser source.” Creators should still use **Show exact template** as the final visual check. A preview requested without that add-on's registered source is reported as a failed verification with recovery instructions instead of a misleading success.
