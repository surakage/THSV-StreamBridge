...
The capability broker turns these declarations into frozen, narrowly scoped runtime handles for private state, bounded scheduling, exact creator-approved Streamer.bot action IDs, source/selected-platform outbound messages, namespaced publication to a core-hosted add-on overlay, and bounded Viewer Foundation projections/mutations. Overlay publishers should pass an explicit third argument such as `{ lane: 'foreground' }`, `{ lane: 'media' }`, `{ lane: 'timer' }`, `{ lane: 'persistent' }`, `{ lane: 'preview' }`, or `{ lane: 'independent' }`; only foreground work enters the shared transient-card queue. Queue acceptance completes immediately, while host diagnostics and logs own later dispatch failures, preventing an earlier long card from timing out the add-on event handler. Viewer consumers receive no salt, raw account-link table, or backing-state access; calls fail closed when the provider or consumer stops. Consumers may subscribe to pseudonymous Viewer Foundation deletion notices so their own caches are removed promptly, but only the official provider can publish those notices. Executable event subscribers must declare `events.subscribe`. See [Add-on capability broker](add-on-capabilities.md).
...
Packages have two compatibility boundaries. `minimumCoreVersion` and `maximumTestedCoreVersion` describe the stable add-on API contract. `minimumBridgeVersion` and `maximumTestedBridgeVersion` describe the actual StreamBridge release whose host routes and relay behavior the add-on needs. Official packages always declare both bridge fields, and the installer rejects a package that is newer or older than its supported host release. Legacy third-party v2 packages without the bridge fields remain readable, but should add them before publishing an update. Packages also declare dependencies, required platform capabilities, event subscriptions, provided commands/actions, owned state, installation/removal instructions, and health checks. Add-ons requesting `overlay.publish` receive the fixed core route `/overlay/addons/<module-id>`; they cannot inject custom browser HTML or JavaScript. Core may expose a shorter alias for a bundled official add-on, but third-party packages must use their canonical module-ID route. `browserSourcesProvided` remains reserved for a future declarative route manifest and must stay empty in this preview.
