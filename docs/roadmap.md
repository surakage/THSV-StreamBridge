...
- Any future Bloom controls for platform moderators must be a separate role-authorized, auditable chat feature. Milestone 10's local control-token endpoint remains operator/streamer-only and must not be exposed as a moderator bypass.
...
11. Installer and Public Release — complete
