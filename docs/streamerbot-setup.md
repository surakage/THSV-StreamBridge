...
12. Import `packages\streamerbot\companion-actions\THSV-StreamBridge-Bloom-Companion-1.1.0.sb` and add it as another immediate receiver child action. It handles only derived `companion.action` events.
...
Multi-Chat, Multi-Commands, Multi-Alerts, Multi-Timed Actions, Viewer Progression, and Bloom Companion must not have direct triggers. Their trust boundary depends on running after a successful receiver invocation. See their adjacent package guides for contracts and QA instructions.
