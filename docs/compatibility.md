...
| Streamer.bot | Minimum and recommended `1.0.7` stable; exact `1.1.0-alpha.3` through `1.1.0-alpha.6` and `1.1.0-alpha.10` schemas embedded | All regenerated THSV package manifests retain stable 1.0.7 as their installation floor. Automated trigger repair is write-enabled only for exact embedded registries or a provenance-verified compatibility-data alias. The creator host's alpha.11 saved actions schema matched its pre-update backup across all 34 managed triggers, but alpha.11 remains inspection-only until its data-only feed is published and verified. Source and automated compatibility checks cover the EventSub chat migration and keep the relay isolated from raw provider WebSocket payloads; repeat the focused live matrix before recording provider acceptance for an alpha host. |
...
Do not infer a future version range from verified alpha builds. Current packages use stable 1.0.7 as their common installation floor, while the rollback-protected trigger reconciler recognizes only exact tested registries. The 1.1.0-alpha.3 registry came from a sanitized capture of the creator host's three THSV intake actions and all 29 managed trigger records; the installed alpha.4 database was then compared read-only and matched that exact schema. For alpha.5, the creator host's imported database was compared read-only against the pre-import alpha.4 backup. The creator host's automatic pre-upgrade alpha.5 and post-upgrade alpha.6 backups were then compared read-only: top-level keys, action-object keys, managed action IDs, enabled states, trigger-record keys, and all 29 trigger types matched. Alpha.10 was separately embedded after exact-schema validation. The alpha.11 data-only alias is documented in [its evidence record](streamerbot-1.1.0-alpha.11.md). Unknown future builds fail closed. After upgrading Streamer.bot, compile every installed THSV C# action, repeat relay tests, verify Action History fields, validate the saved actions schema, and then update the live-verification record. Re-import the current versioned packages when replacing an older baseline export.
...
The September 22 alpha.12 compatibility-data candidate is documented in [its evidence record](streamerbot-1.1.0-alpha.12.md). Its complete pre/post-upgrade actions database matched after excluding only the save timestamp. Like alpha.11, it requires verified feed delivery before installed trigger repair is enabled.
...
See [Streamer.bot 1.1.0 alpha.6 compatibility](streamerbot-1.1.0-alpha.6.md) for the latest exact-version evidence and remaining live acceptance boundary, and [Streamer.bot 1.0.7 stable adoption](streamerbot-1.0.7-stable.md) for the recommended stable baseline. The [alpha.5](streamerbot-1.1.0-alpha.5.md), [1.0.6](streamerbot-1.0.6-stable.md), [1.0.5 stable](streamerbot-1.0.5-stable.md), [beta.5](streamerbot-1.0.5-beta.5.md), and [beta.2](streamerbot-1.0.5-beta.2.md) records remain historical evidence rather than current install guidance.
