...
Do not infer a future version range from verified alpha builds. Current packages use stable 1.0.7 as their common installation floor, while the rollback-protected trigger reconciler recognizes only exact tested registries. The 1.1.0-alpha.3 registry came from a sanitized capture of the creator host's three THSV intake actions and all 29 managed trigger records; the installed alpha.4 database was then compared read-only and matched that exact schema. Alpha.5 and future builds fail closed. After upgrading Streamer.bot, compile every installed THSV C# action, repeat relay tests, verify Action History fields, validate the saved actions schema, and then update the live-verification record. Re-import the current versioned packages when replacing an older baseline export.
