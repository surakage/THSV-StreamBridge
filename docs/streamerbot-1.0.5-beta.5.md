...
No normalized event, add-on API, WebSocket request, C# reference, trigger, or package-manifest change is required for beta.5. Existing THSV `2.5.1` imports do not need to be re-imported solely because Streamer.bot was upgraded, and package minimum versions remain at their oldest verified compatible floors.
