...
For a normal move to another computer, use `Move THSV StreamBridge to Another Computer.cmd` instead. Its encrypted `.thsv-transfer` bundle carries configuration, non-secret runtime state, installed extension packages, add-on settings/state, scene evidence, and creator assets. Sensitive JSON fields, secret/private/vault paths, provider credentials, WebSocket passwords, tokens, API keys, and webhook URLs are excluded. On the destination computer, install the same or newer verified Bridge, run `Restore THSV Move Bundle.cmd`, then re-enter credentials and test connections in the Wizard. The restore is transactionally approval-gated and never replaces the destination installation's secrets directory.

Before a move export asks for a passphrase or stops StreamBridge, the launcher prints the exact included filenames and sizes, redacted credential-field count, omitted secret/private-file count, and categories that are always excluded. Type `MOVE` only after reviewing this inventory. The completed transfer bundle is decrypted, authenticated, and structurally verified before its saved-path success message appears.
