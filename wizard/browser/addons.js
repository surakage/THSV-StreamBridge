...
function renderAddOnTriggerReadiness(addOn) {
  ...
  if (addOn.moduleId === 'thsv.village-roll-call') return `<details class="form-section addon-trigger-readiness addon-step" data-disclosure-key="addon:thsv.village-roll-call:trigger-readiness"><summary><span><span class="step-number">3</span><strong>Connect one Twitch reward</strong><small>The existing Twitch intake receives each monthly check-in.</small></span><span class="status-chip status-ready">No add-on import needed</span></summary><div class="addon-step-body"><ol><li>Create the Channel Points reward in <strong>Streamer.bot</strong>, not Twitch, so Streamer.bot can manage it reliably.</li><li>On the main <strong>THSV Twitch - Intake</strong> action, keep one <strong>Reward Redemption (Any Reward)</strong> trigger and enable <strong>Skip Reward Queue</strong>.</li><li>Test the reward once, copy its stable reward ID from the action arguments, and paste that exact ID into Village Roll Call settings.</li><li>Save, restart StreamBridge, and use the local preview. Real monthly records accept only verified, non-simulated Twitch redemptions for that reward ID.</li></ol><p class="notice"><strong>No second redemption trigger:</strong> adding one directly to this add-on would process the same redemption twice.</p></div></details>`;
