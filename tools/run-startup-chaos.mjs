...
const scenarios = Object.freeze(['early process exit and retry', 'occupied listener ownership', 'crash-loop circuit breaker', 'transactional installer rollback']);
...
const exitCode = await new Promise((resolveExit, rejectExit) => { child.once('error', rejectExit); child.once('exit', (code) => resolveExit(code ?? 1)); });
