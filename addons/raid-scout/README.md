...
- Can optionally start or adopt one ending Twitch ad as Suggest begins, continue discovery, confirmation, and full clip playback during it, use the genuine Twitch Ad Run duration as its timer, and then run the selected Stop Streaming action only after the raid is accepted. The same flow never requests a duplicate commercial. A fixed countdown remains available as an advanced fallback.
...
7. Configure preferred channels and filters, then use Suggest while live. To start automatically at the end of a stream, enable the ending-scene option and enter the exact OBS scene name (including any emoji or folder symbol).
...
In the recommended after-ad mode, Suggest or Finish Stream dispatches the included Run Ending Ad action immediately while Twitch is still live. Discovery and manual or automatic confirmation proceed during the commercial; after confirmation, the selected clip plays through completion, then Raid Scout starts the raid. It tracks the exact genuine `adLength` reported by Streamer.bot and stops outputs only after both raid acceptance and the ad end plus the configured buffer. If Twitch rejects the ad or no real Ad Run event arrives, the destination search and raid continue but every broadcast remains live. The OBS/Aitum helper recognizes Aitum's stable `aitum_multi_output_` names and its RTMP, WHIP, FTL, or MPEG-TS transports, requests every matching output to stop, and gives all secondary outputs one shared three-second confirmation window. OBS main is then stopped unconditionally; an Aitum failure is logged but can never hold the main broadcast online. It does not target recording, replay buffer, or virtual camera. Cancel can abort before the raid or selected stop action runs. Search failure, clip failure by itself, raid rejection, simulated ad/control events, missing approval, or a missing safety acknowledgement cannot stop the broadcast. After dispatch, Raid Scout waits for the imported provider confirmation action; if no confirmation arrives, it tells the creator to stop manually and never retries.
