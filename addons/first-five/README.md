...
Import `packages/streamerbot/first-five/THSV-StreamBridge-First-Five-4.0.11.sb`. Keep **THSV Addon - First Five - Controller** triggerless and approve only that stable action ID. Create `!firstfive` with no generated response for YouTube and TikTok. The optional Reset action may be attached to a deck button or hotkey.
