...
const manifest = {
  contractVersion: '2.0.0-preview.1', moduleId: MODULE_ID, name: 'Custom Counter', version: '3.0.0',
  ...
  installationSteps: ['Install Custom Counter and import its separate Streamer.bot controls.', 'Create the optional no-response counter command through Command Sync.', 'Attach creator controls only to trusted hotkeys, Stream Deck buttons, or manual triggers.', 'Add /overlay/addons/thsv.custom-counter as a browser source.'],
...
function controlFor(event, settings) {
  ...
  if (event.eventType !== 'command.received' || !settings.commandEnabled || counterId(event.payload?.command) !== settings.commandName || !creator(event, settings)) return undefined;
...
async function applyControl(control, event, context) { const settings = settingsFor(context); let state = stateFor(await context.state.read(), settings); if (control.invalid) return reply(context, event, `Usage: !${settings.commandName} [counter] [show|hide|+1|-1|add|subtract|set|reset] [amount]`);
