...
export async function processChatGuardEvent(event, context, now = Date.now()) {
  ...
  const state = sanitizeState(await context.state.read(), settings, now); if (settings.ignoredAccounts.has(stableAccount) || state.trustedAccounts.some((item) => item.platform === event.platform && item.userId.toLowerCase() === clean(user.id, 256).toLowerCase())) return undefined; const eventHash = await digest(`${state.salt}|event|${clean(event.eventId, 256)}`);
