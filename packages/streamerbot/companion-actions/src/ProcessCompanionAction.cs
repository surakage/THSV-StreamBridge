...
public class CPHInline
{
    ...
    public bool Execute()
    {
        ...
        if (action != "wave" && action != "eat" && action != "sleep" && action != "celebrate") return Fail("payload.action is not a supported Bloom action.");
        if (string.IsNullOrWhiteSpace(actorName) || actorName.Length > 256) return Fail("payload.actorName is invalid.");
        if (!ReadInteger(payload, "cost", 0, 1000000, out long cost) || !ReadInteger(payload, "remainingPoints", 0, 9007199254740991L, out long remaining) || !ReadInteger(payload, "happiness", 0, 100, out long happiness) || !ReadInteger(payload, "fullness", 0, 100, out long fullness) || !ReadInteger(payload, "energy", 0, 100, out long energy)) return Fail("Companion counters are invalid.");
        ...
        CPH.SetArgument("companionHappiness", happiness); CPH.SetArgument("companionFullness", fullness); CPH.SetArgument("companionEnergy", energy); CPH.SetArgument("companionSimulated", ReadArgumentBoolean("streamBridgeSimulated"));
    ...
    private void InitializeOutputs()
    {
        ...
        CPH.SetArgument("companionCost", 0L); CPH.SetArgument("companionRemainingPoints", 0L); CPH.SetArgument("companionHappiness", 0L); CPH.SetArgument("companionFullness", 0L); CPH.SetArgument("companionEnergy", 0L); CPH.SetArgument("companionSimulated", false);
    }
