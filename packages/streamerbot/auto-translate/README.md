...
Import `THSV-StreamBridge-Auto-Translate-2.4.0.sb`, compile the single action with the references declared in `manifest.json`, and approve that exact action for Auto Translate in the StreamBridge wizard. Do not attach a trigger; the add-on capability broker dispatches it.
