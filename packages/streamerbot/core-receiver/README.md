...
2. Drag `THSV-StreamBridge-Core-Receiver-1.0.1.sb` into the import field, or paste the file contents.
...
The exported action uses Streamer.bot's **Default** queue with concurrent execution disabled. The bridge controls ingress concurrency and queue capacity; the receiver remains stateless per invocation. The C# source depends on the bundled CPH API and Streamer.bot's default Newtonsoft.Json/JObject reference. Package `1.0.1` was compiled against Streamer.bot `1.0.5-alpha.31`.
