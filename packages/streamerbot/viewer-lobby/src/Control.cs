...
public class CPHInline
{
    public bool Execute()
    {
        ...
        string[] allowed = { "open", "close", "pause", "resume", "next", "random", "complete", "clear" }; if (Array.IndexOf(allowed, action) < 0) return false;
        var envelope = new JObject { ["type"] = "thsv.addon", ["version"] = "1.0.0", ["moduleId"] = "thsv.viewer-lobby", ["eventType"] = "addon.thsv.viewer-lobby.control", ["sourceEventType"] = "THSV Addon - Viewer Lobby - " + Label(action), ["relayId"] = Guid.NewGuid().ToString("N"), ["relayToken"] = "", ["receivedAt"] = DateTimeOffset.UtcNow.ToString("O"), ["simulated"] = ReadBool("isTest"), ["payload"] = new JObject { ["action"] = action } };
